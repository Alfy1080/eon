# E·ON România — Home Assistant Integration

[![Home Assistant](https://img.shields.io/badge/Home%20Assistant-2024.2%2B-41BDF5?logo=homeassistant&logoColor=white)](https://www.home-assistant.io/)
[![HACS Custom](https://img.shields.io/badge/HACS-Custom-41BDF5.svg)](https://github.com/hacs/integration)
[![GitHub Release](https://img.shields.io/github/v/release/developer/ha-eon-romania)](https://github.com/developer/ha-eon-romania/releases)
[![GitHub Stars](https://img.shields.io/github/stars/developer/ha-eon-romania?style=flat&logo=github)](https://github.com/developer/ha-eon-romania/stargazers)

Custom integration for [Home Assistant](https://www.home-assistant.io/) that monitors contract data, consumption, and bills through the [E·ON România](https://www.eon.ro/) API (E·ON Myline mobile app).

It provides dedicated sensors per billing code for contract, current index, balance, invoices, payments, reading history, consumption agreement, allowed reading, and buttons to submit the index per utility. Fully supports DUO contracts (collective gas + electricity).

---

## What the integration does

- **Automatic discovery** of contracts associated with the E·ON Myline account
- **Granular selection** of the contracts you want to monitor (checkboxes with full addresses)
- **Multi-contract** — a single E·ON account can monitor multiple billing codes simultaneously
- **Full DUO support** — collective contracts (gas + electricity under a single code) are automatically detected, with separate sensors per subcontract (index, reading allowed, consumption agreement)
- **Dedicated sensors** per selected contract — each contract becomes a dedicated device
- **Balance and invoices** — balance existence (Yes/No), overdue invoices with details in attributes, Romanian format (1.234,56 lei)
- **Reading history** — monthly indexes with reading type (self-read / estimated / distributor-read)
- **Payment archive** — payments made per year, with an annual total
- **Consumption archive** — monthly and daily average consumption per year, with the Romanian decimal separator (comma)
- **Normalized addresses** — correct formatting from API data into Romanian format
- **County mapping** — short codes (AB, BV, CJ) automatically converted to full names (Alba, Brașov, Cluj)
- **Licensing system** — without a valid license, only the "License required" sensor is displayed
- **Reconfiguration without reinstallation** — OptionsFlow for modifying credentials and contract selection

---

## Data Source

The data comes from the E·ON Myline mobile application API (`api2.eon.ro`), which exposes REST endpoints for:

| Endpoint | Descriere |
|----------|-----------|
| contract-details | Contract details (prices, address, PCS) |
| account-contracts/list | List of subcontracts (for DUO contracts) |
| invoices/list | Unpaid invoices |
| invoices/list-prosum | Prosumer invoices |
| invoice-balance | Invoice balance |
| invoice-balance-prosum | Prosumer balance |
| rescheduling-plans | Rescheduling plans |
| graphic-consumption | Annual consumption graph (consumption archive) |
| meter-reading/index | Current meter index |
| meter-reading/history | Meter reading history |
| consumption-convention | Consumption agreement |
| payments/payment-list | Payment history |

Authentication is done with email + password + HMAC-MD5 signature (mobile-login). Expired tokens (401) are automatically renewed.
...
3. Add the URL: `https://github.com/developer/ha-eon-romania`
4. Category: **Integration**
5. Click **Add** → find "E·ON România" → **Install**
6. Restart Home Assistant

### Manual

1. Copy the `custom_components/eonromania/` folder into the `config/custom_components/` directory of your Home Assistant
...
2. Search for "**E·ON România**"
3. Fill out the form:

| Field | Description | Default |
|---|---|---|
| **Email** | E·ON Myline account email address | — |
| **Password** | E·ON Myline account password | — |
| **Update interval** | Seconds between API queries | `3600` (1 hour) |

### Step 2 — Select contracts

After authentication, contracts are automatically discovered. Each contract appears with its full normalized address:
